...
/* Shamelessly stolen from asynManager.c */
    ...
    while(TRUE) {
        ...
        if (!in_sync) {     /* Try to resynchronize! */
            ...
            if (LCLS2_FID_DIFF(delayfid, tsfid) > sync_far) {
                SYNC_ERROR(0, ("%s is still way off! delayfid = 0x%lu, tsfid = 0x%lu, restarting.\n",
                               Name(), delayfid, tsfid));
            }
        ...
        if (m_gen) {
            ...
            if (tsfid == TIMING_PULSEID_INVALID) {
                SYNC_ERROR(0, ("Invalid fiducial! (delayed fid 0x%lu)\n", delayfid));
            }
            ...
            if (attributes & SyncObject::HasTime) {
                ...
                if (tsfid == TIMING_PULSEID_INVALID) {
                    SYNC_ERROR(0, ("Invalid fiducial! (expected fid 0x%lu)\n", fid));
                }
                /* We're keeping it tight here for the initial sync! */
                if (abs(LCLS2_FID_DIFF(fid, tsfid)) >= sync_far) {
                    SYNC_ERROR(0, ("Lost sync! (timestamp fid 0x%lu, expected fid 0x%lu)\n", tsfid, fid));
                }
            }
            ...
            if (attributes & SyncObject::CanSkip) {
                ...
                if (LCLS2_FID_DIFF(delayfid, tsfid) >= sync_vfar) {
                    SYNC_ERROR(0, ("Lost sync! (timestamp fid 0x%lu, delayed fid 0x%lu, last tsfid = 0x%lu, last delayfid = 0x%lu)\n", tsfid, delayfid, lasttsfid, lastdelayfid));
                }
            }
            lasttsfid = tsfid;
                ...
                if (abs(LCLS2_FID_DIFF(tsfid, delayfid)) < sync_notfar) {
                    ...
                    if (SYNC_DEBUG(0)) {
                        ...
                        if (!do_print)
                            ...
                            printf("%s has data at fiducial 0x%lu (0x%lu - %lg = 0x%lu).\n",
                    ...
                    SYNC_ERROR(0, ("%s has lost synchronization! (timestamp fid = 0x%lu, delay fid = 0x%lu, diff = %d)\n",
